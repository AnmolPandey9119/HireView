/* ════════════════════════════════════════════════
   ...
   async function startRound() {
     ...
     // Requested synchronously, before any await below — browsers only
     // honour a fullscreen request while it's still inside the original
     // click's "user gesture" window, which an await would otherwise break.
     if (typeof FullscreenGuard !== 'undefined') {
       FullscreenGuard.start({
         maxTabSwitches: 2,
         maxWindowBlur: 2,
         onWarn: (reason, count) => {
           if (reason === 'tab_switch') showToast(`⚠️ Tab switch detected (${count}/2) — stay on this page.`, true);
           else if (reason === 'window_blur') showToast(`⚠️ Window switch detected (${count}/2).`, true);
         },
         onTerminate: (reason) => terminateRoundForCheating(reason),
       });
     }
   ...
   // ────────────────────────────────────────────
   // Cheating termination — fired by FullscreenGuard when the candidate
   // exceeds the fullscreen-exit / tab-switch / window-blur strikes.
   // Grades whatever was submitted so far (same aggregation as a normal
   // finish) but marks the attempt cheating_terminated on the backend so
   // My Reports and admin can tell the difference.
   // ────────────────────────────────────────────
   async function terminateRoundForCheating(reason) {
     if (crBusy) return;
     crBusy = true;
     ...
     try {
       const result = await apiPost(`/api/coding/${crAttemptId}/terminate`, {
       ...
       console.error(err);
       showToast('Round ended due to an integrity violation.', true);
       showScreen('setup');
     } finally {
       crBusy = false;
     }
   ...
   document.getElementById('crSubmitBtn').addEventListener('click', async () => {
     ...
     try {
       ...
       // Response recorded — now move on automatically. A short pause
       // lets the candidate actually see the pass/fail banner before the
       // next problem replaces it.
       const isLast = crCurrentIndex === crQuestions.length - 1;
       btn.textContent = 'Recorded — Moving on…';
       showToast(result.is_solved ? '✅ Solved! Your answer is recorded.' : `${result.passed_count}/${result.total_count} test cases passed — answer recorded.`, !result.is_solved);
       ...
       return; // crBusy/btn reset happens above, inside the timeout
     } catch (err) {
       console.error(err);
       wrap.innerHTML = '';
   ...
   function renderResults(result) {
     ...
     const terminated = result.status === 'cheating_terminated';
     const terminatedBanner = terminated ? `
       <div class="cr-result-banner fail" style="margin-bottom:1rem;">
         🚨 Round Ended Early — Integrity Violation Flagged
       </div>
     ` : '';
