/* ════════════════════════════════════════════════
   ...
   async function startTest() {
     ...
     if (typeof FullscreenGuard !== 'undefined') {
       FullscreenGuard.start({
         ...
         onTerminate: (reason) => terminateTestForCheating(reason),
       });
     }
   ...
   // Submits whatever answers were selected so far, but marks the
   // attempt cheating_terminated on the backend instead of completed.
   // ────────────────────────────────────────────
   async function terminateTestForCheating(reason) {
     ...
     try {
       const result = await apiPost(`/api/aptitude/${aptAttemptId}/terminate`, {
         answers,
         time_taken_seconds: Math.round((Date.now() - aptStartedAt) / 1000),
         reason,
       });
       ...
       console.error(err);
       showToast('Test ended due to an integrity violation.', true);
       showScreen('setup');
     } finally {
       aptSubmitting = false;
     }
   ...
   function renderResults(result) {
     ...
     const terminatedBanner = terminated ? `
       <div class="apt-diff" style="display:block;text-align:center;margin-bottom:1rem;padding:0.6rem;border-radius:var(--hv-radius-md,12px);background:rgba(239,68,68,0.14);color:var(--hv-error-light,#f87171);font-weight:800;">
         🚨 Test Ended Early — Integrity Violation Flagged
       </div>
     ` : '';
