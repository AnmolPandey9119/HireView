...
function showQuotaExceededError(sector, msg, showPlansLink = true) {
  ...
  el.innerHTML = `
    ...
    ${showPlansLink ? `<button onclick="window.location.href='/dashboard?openPayment=1'"
      style="margin-top:0.75rem;padding:0.6rem 1.4rem;background:linear-gradient(135deg,#C65D3A,#E8935F);border:none;border-radius:10px;color:white;font-weight:700;cursor:pointer;font-family:inherit;font-size:0.85rem">
      View Plans →
    </button>` : ''}`;
  el.classList.add('show');
}
