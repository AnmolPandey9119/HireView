...
(function () {
  ...
  function mountButton(btn) {
    ...
    //   - .sidebar-nav  = the hidden nav inside the account-settings
    //     overlay panel (invisible until that panel is opened)
    //   - .app-sidebar-nav = the persistent left app sidebar
    //     (js/sidebar.js) added on dashboard/history — real and
    //     visible, but it's a feature-navigation rail, not the site's
    //     top navbar, so the button doesn't belong inside it.
    var navLinks = document.querySelector(".nav-links");
    var nav = document.querySelector("nav:not(.sidebar-nav):not(.app-sidebar-nav)");
