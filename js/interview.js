...
async function processResumeFile(file) {
  ...
  try {
    ...
    showSetupError('Failed to read the file. Please try a PDF or TXT.');
    document.getElementById('resumeParsingIndicator').style.display = 'none';
  }
}
...
async function handleInterviewStart() {
  ...
  selectedLanguage = document.getElementById('interviewLanguage').value;

  if (!jobTitle) { showSetupError('Please enter a job title.'); return; }
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/api/interviews`, {
      ...
      body: JSON.stringify({ role: jobTitle, difficulty: 'adaptive', duration_limit: 3600 })
    });
...
async function handleRecordingChoice(choice, btn) {
  ...
  if (choice === 'download' && blob) {
    ...
    const role = document.getElementById('jobTitle')?.value || 'interview';
    a.href = url;
